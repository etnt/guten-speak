...
#include "pocket_tts_custom_attention.hpp"
    ...
    struct Tensor
    {
        ...
        Tensor squeeze(int64_t dim = -1) const
        {
            ...
            for (size_t i = 0; i < shape.size(); ++i)
                if (shape[i] != 1 || (dim >= 0 && (int64_t)i != dim))
        ...
        static Tensor concat(const std::vector<Tensor> &ts, int64_t dim)
        {
            ...
            for (int64_t i = 0; i < dim; ++i)
                outer *= os[i];
            for (size_t i = dim + 1; i < os.size(); ++i)
                inner *= os[i];
    ...
    static std::vector<float> resample(const std::vector<float> &in, int src, int dst)
    {
        ...
        auto sinc = [&](float x)
        { return std::abs(x) < 1e-6f ? 1.0f : std::sin(PI * x) / (PI * x); };
        auto lanczos = [&](float x)
        { return std::abs(x) >= K ? 0.0f : sinc(x) * sinc(x / K); };
    ...
    static std::vector<std::string> split_sentences(const std::string &text)
    {
        ...
        for (size_t i = 0; i < text.size(); ++i)
        {
            ...
            if ((text[i] == '.' || text[i] == '!' || text[i] == '?'))
            {
                ...
                if (text[i] == '.' && i > 0 && text[i - 1] == '.')
                    continue;
                if (text[i] == '.' && is_abbreviation(text, i))
                    continue;
        ...
        for (const auto &sentence : sentences)
        {
            ...
            {
                while (start < sentence.size() && std::isspace((unsigned char)sentence[start]))
                    start++;
                if (sentence.size() - start <= MAX_CHUNK_CHARS)
                {
                    ...
                    if (trim != std::string::npos)
                        chunks.push_back(piece.substr(trim));
                    break;
                }
                ...
                if (split == std::string::npos)
                {
                    for (size_t i = limit; i > start; --i)
                    {
                        if (std::isspace((unsigned char)sentence[i - 1]))
                        {
                            split = i - 1;
                            break;
                        }
                ...
                if (trim != std::string::npos)
                {
                    ...
                    while (!piece.empty() && std::isspace((unsigned char)piece.back()))
                        piece.pop_back();
                    chunks.push_back(piece);
                }

                start = split;
                while (start < sentence.size() && (std::isspace((unsigned char)sentence[start]) || sentence[start] == ',' || sentence[start] == ';' || sentence[start] == ':'))
                    start++;
            }
        }
    ...
    namespace cache
    {
        ...
        static std::string get_cache_path(const std::string &voices_dir, const std::string &voice_path, const char *ext = "emb")
        {
            ...
            if (slash != std::string::npos)
                filename = voice_path.substr(slash + 1);
    ...
    static std::basic_string<ORTCHAR_T> to_ort_path(const std::string &s)
    {
        ...
        if (n <= 0)
            throw std::runtime_error("Failed to widen path: " + s);
        std::wstring w(n - 1, 0);
    ...
    public:
            ...
            : sess_(env, to_ort_path(path).c_str(), opts), name_(name.empty() ? path : name)
        {
            ...
            for (const auto &n : in_names_)
                in_ptrs_.push_back(n.c_str());
            for (const auto &n : out_names_)
                out_ptrs_.push_back(n.c_str());
        }
    ...
    struct StateBufferIO
    {
        ...
        void copy_delta_f32(size_t state_idx, int64_t old_step, int64_t new_step, Ort::Value &val)
        {
            ...
            int capacity_axis = -1;
            if (full_shape[2] == 1000)
                capacity_axis = 2;
            else if (full_shape[3] == 1000)
                capacity_axis = 3;
            if (capacity_axis < 0)
            {
            ...
            if (dst_vec.size() != full_size)
                dst_vec.resize(full_size, 0.0f);

            const float *src = val.GetTensorData<float>();
            float *dst = dst_vec.data();
            int64_t capacity = full_shape[capacity_axis];
            int64_t inner = 1;
            for (int d = capacity_axis + 1; d < 5; ++d)
                inner *= full_shape[d];
            int64_t outer = 1;
            for (int d = 0; d < capacity_axis; ++d)
                outer *= full_shape[d];
        ...
        struct DiskSnapshot
        {
            ...
            bool save_to_disk(const std::string &path) const
            {
                size_t slash = path.find_last_of('/');
                if (slash != std::string::npos)
                    cache::mkdir_p(path.substr(0, slash));
                std::ofstream f(path, std::ios::binary);
                if (!f)
                    return false;
            ...
            for (size_t i = 0; i < n; ++i)
            {
                ...
                if (slices[i].seq_dim >= 0)
                {
                    ...
                    for (int d = 0; d < sd; ++d)
                        outer *= shapes[i][d];
                    int64_t inner = 1;
                    for (size_t d = sd + 1; d < shapes[i].size(); ++d)
                        inner *= shapes[i][d];
                ...
                else
                {
                    ...
                    if (!f32[fb][i].empty())
                    {
                        memcpy(snap.f32_data.data() + fo, f32[fb][i].data(), f32[fb][i].size() * sizeof(float));
                        fo += f32[fb][i].size();
                    }
                    if (!f16[0][i].empty())
                    {
                        memcpy(snap.f16_data.data() + ho, f16[0][i].data(), f16[0][i].size() * sizeof(uint16_t));
                        ho += f16[0][i].size();
                    }
                }
                ...
                if (!i64[b][i].empty())
                {
                    memcpy(snap.i64_data.data() + io, i64[b][i].data(), i64[b][i].size() * sizeof(int64_t));
                    io += i64[b][i].size();
                }
                if (!b8[b][i].empty())
                {
                    memcpy(snap.b8_data.data() + bo, b8[b][i].data(), b8[b][i].size());
                    bo += b8[b][i].size();
                }
            }
        ...
        void restore_snapshot(const Snapshot &snap)
        {
            ...
            for (size_t i = 0; i < n; ++i)
            {
                ...
                {
                    size_t full_size = 1;
                    for (auto d : init_shapes[i])
                        full_size *= (d > 0 ? d : 1);

                    int sd = -1;
                    for (size_t d = 0; d < init_shapes[i].size(); ++d)
                    {
                        if (snap.shapes[i][d] != init_shapes[i][d])
                        {
                            sd = (int)d;
                            break;
                        }
                    ...
                    if (has_f16)
                    {
                        ...
                        if (sd >= 0)
                        {
                            ...
                            int64_t outer = 1;
                            for (int d = 0; d < sd; ++d)
                                outer *= init_shapes[i][d];
                            int64_t inner = 1;
                            for (size_t d = sd + 1; d < init_shapes[i].size(); ++d)
                                inner *= init_shapes[i][d];
            ...
            for (int32_t i = 0; i < ns; ++i)
            {
                ...
                else if (type == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16)
                {
                    ...
                    if (sliced)
                    {
                        ...
                        for (size_t d = 0; d < init_shapes[i].size(); ++d)
                        {
                            if (loaded_shape[d] != init_shapes[i][d])
                            {
                                sd = (int)d;
                                break;
                            }
    ...
    class StatefulRunner
    {
        ...
        void discover_delta_kv_states()
        {
            ...
            auto is_delta_output = [&](size_t state_idx, int capacity_axis)
            {
                ...
                {
                    if (out_names[oi] != out_name || out_types[oi] != ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT)
                        continue;
    ...
    public:
              ...
              single_buffer_large_f32_cache_(single_buffer_large_f32_cache)
        {
            ...
            {
                if (state_.types[i] != ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16)
                    continue;
                AttnLayer l{};
                l.k = i;
                for (size_t j = i + 1; j < state_.names.size(); ++j)
                    if (state_.types[j] == ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16)
                    {
                        l.v = j;
                        break;
                    }
                for (size_t j = l.v + 1; j < state_.names.size(); ++j)
                    if (state_.types[j] == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64)
                    {
                        l.step = j;
                        break;
                    }
    ...
    public:
        explicit Tokenizer(const std::string &path)
        {
            ...
            if (!s.ok())
                throw std::runtime_error("Failed to load tokenizer: " + s.ToString());
        }
        std::vector<int> encode(const std::string &text) const
        {
            std::vector<int> ids;
    ...
    public:
        ...
        explicit PocketTTS(const Config &cfg = {}) : cfg_(cfg)
        {
            ...
            if (cfg_.num_threads > 0)
                g_wasm_pool_threads = cfg_.num_threads;
#endif
            ...
            if (cfg_.decoder_only)
            {
                dec_ = std::make_unique<OrtSession>(env, dec_path, opts_dec,
                                                    dec_custom_convtr ? "mimi_decoder_delta_convtr" + sfx : (dec_custom_attention ? "mimi_decoder_delta_attn" + sfx : (dec_delta_kv ? "mimi_decoder_delta" + sfx : "mimi_decoder" + sfx)));
            ...
            main_ = std::make_unique<OrtSession>(env, main_path, opts_main,
                                                 main_merged_flow ? (main_custom_attention ? "flow_lm_main_delta_attn_flow" + sfx
                                                                  ...
                                                                  : (main_custom_attention ? "flow_lm_main_delta_attn" + sfx : (main_delta_kv ? "flow_lm_main_delta" + sfx : "flow_lm_main" + sfx)));
        ...
        static AudioData load_audio(const std::string &path)
        {
            ...
            float mx = *std::max_element(mono.begin(), mono.end(), [](float a, float b)
        ...
        Tensor encode_voice(const std::string &path)
        {
            ...
            if (a.samples.size() > MAX_VOICE_SAMPLES)
            {
                ...
                if (cfg_.verbose)
                    std::cerr << "  Voice truncated to 30s\n";
            }
            ...
            while (r.shape.size() > 3)
                r = r.squeeze(0);
            if (r.shape.size() < 3)
                r.reshape({1, r.shape[0], r.shape[1]});
        ...
        double warmup()
        {
            ...
            stream("Hi.", dummy_voice, [](const float *, size_t)
                   { return true; }, 1);
            auto end = std::chrono::high_resolution_clock::now();
    ...
    private:
        ...
        {
            if (bos_before_voice_.data.empty())
                return true;
        ...
        {
            if (bos_before_voice_.data.empty() || has_bos_before_voice(voice))
                return voice;
            if (voice.shape.size() != 3 ||
                voice.shape[0] != bos_before_voice_.shape[0] ||
                voice.shape[2] != bos_before_voice_.shape[2])
        ...
        TensorI64 tokenize(const std::string &text)
        {
            ...
            t = t.substr(s, e - s + 1);
            if (std::isalnum((unsigned char)t.back()))
                t += ".";
            if (!t.empty() && std::islower((unsigned char)t[0]))
                t[0] = std::toupper((unsigned char)t[0]);

            auto ids = tok_->encode(t);
            if (cfg_.verbose)
                std::cerr << "  Tokens: " << ids.size() << " from " << t.size() << " chars\n";
        ...
        public:
                  ...
                  main_runner_(*tts.main_runner_)
            {
                ...
                if (out_voice_snap)
                    *out_voice_snap = main_runner_.take_snapshot();
            ...
            {
                if (!has_next())
                    throw std::runtime_error("No more latents");
                ...
                if (merged)
                {
                    ...
                    if (temp_ > 0)
                    {
                        ...
                        if (nc > 0)
                            for (auto &v : fx_)
                                v = std::max(-nc, std::min(nc, v));
                    }
                    else
                        std::fill(fx_.begin(), fx_.end(), 0.0f);
                }
        ...
        static uint64_t voice_hash(const Tensor &v)
        {
            ...
            for (int64_t dim : v.shape)
                mix(static_cast<uint64_t>(dim));
        ...
        LatentGen make_gen(const Tensor &v, const TensorI64 &t, int max, int eos_extra)
        {
            ...
            if (cfg_.voice_cache && !voice_kv_path_.empty() && !voice_embedding_rebuilt_)
            {
                ...
                {
                    if (cfg_.verbose)
                        std::cerr << "  Loaded KV cache: " << kv_path << "\n";
            ...
            if (cfg_.voice_cache && !voice_kv_path_.empty())
            {
                ...
                {
                    if (cfg_.verbose)
                        std::cerr << "  Saved KV cache: " << kv_path << "\n";
                }
            }
    ...
    void PocketTTS::stream(const std::string &text, const Tensor &voice, StreamCallback cb, int max_frames)
    {
        ...
        {
            if (fade_in_pos >= fade_in_total || count == 0)
                return;
        ...
        auto emit_audio = [&](float *data, size_t count) -> bool
        {
            ...
            if (trimming_leading)
            {
                ...
                if (!leading_candidate.empty())
                {
                    ...
                    if (!cb(leading_candidate.data(), leading_candidate.size()))
                        return false;
                    leading_candidate.clear();
                }
        ...
        {
            if (!trimming_leading || leading_candidate.empty())
                return true;
            trimming_leading = false;
            fade_in_total = SENTENCE_FADE_SAMPLES;
            fade_in_pos = 0;
        ...
        for (size_t si = 0; si < sentences.size(); ++si)
        {
            ...
            while (true)
            {
                ...
                bool final_batch = false;
                {
                    ...
                    cv.wait(lock, [&]
                            { return (int)queue.size() >= want || gen_done || aborted; });
                    if (aborted)
                        break;
                ...
                if (take > 0)
                {
                    ...
                    if (first_decoder)
                        trace.mark("first_decoder_start");
            ...
            if (si + 1 < sentences.size())
            {
                ...
                if (!pending_tail.empty())
                {
                    ...
                    if (!emit_audio(pending_tail.data(), pending_tail.size()))
                        return;
                    pending_tail.clear();
                }
                size_t pause = terminal ? sentence_pause.size() : size_t(CHUNK_PAUSE_SAMPLES);
                if (pause > 0 && !cb(sentence_pause.data(), pause))
                    return;
                fade_in_total = SENTENCE_FADE_SAMPLES;
                fade_in_pos = 0;
    ...
    struct HttpRequest
    {
        ...
        static HttpRequest parse(ptt_socket_t client_fd, std::string &buf)
        {
            ...
            while ((header_end = buf.find("\r\n\r\n")) == std::string::npos)
            {
                ...
                if (n <= 0)
                    return req; // closed, reset, or idle timeout
                buf.append(tmp, n);
            }
        ...
        for (size_t i = pos + 1; i < json.size(); ++i)
        {
            if (json[i] == '\\' && i + 1 < json.size())
            {
                ...
                else if (next == 'u' && i + 5 < json.size())
                {
                    ...
                    {
                        result += '\\';
                        result += next; // malformed, pass through
                    }
                }
                else
                {
    ...
    public:
        ...
        void run()
        {
            while (g_server_running)
            {
                ...
                if (client_fd == PTT_INVALID_SOCKET)
                    break;
    ...
    private:
        void serve_connection(ptt_socket_t client_fd)
        {
            ...
            while (g_server_running)
            {
                ...
                if (!req.valid)
                    break;
                if (!handle_request(client_fd, req))
                    break;
                if (req.want_close)
                    break;
            }
        ...
        {
            std::string status_text = (status == 200) ? "OK" : (status == 404) ? "Not Found"
                                                                               : "Bad Request";
        ...
        static std::vector<uint8_t> wav_encode(const float *samples, size_t count, int sample_rate)
        {
            ...
            auto w = [&](size_t off, const void *src, size_t n)
        ...
        bool send_chunk(ptt_socket_t fd, const void *data, size_t len)
        {
            ...
            if (!ptt_send(fd, size_buf, strlen(size_buf)))
                return false;
            if (!ptt_send(fd, data, len))
                return false;
            return ptt_send(fd, "\r\n", 2);
        }
...
extern "C"
{
    ...
    {
        if (!handle || !latents || frames <= 0 || !out_samples || !out_len)
            return -1;
        try
        {
    ...
    {
        if (handle)
            static_cast<pocket_tts::PocketTTS *>(handle)->reset_decoder();
    }

    void ptt_set_temperature(void *handle, float temperature)
    {
        if (handle)
            static_cast<pocket_tts::PocketTTS *>(handle)->set_temperature(temperature);
    }
...
#ifdef __EMSCRIPTEN__
        if (threads > 0)
            pocket_tts::g_wasm_pool_threads = threads;
                              ...
                              const char *profile_dir)
    {
        try
        {
            ...
            cfg.num_threads = num_threads;
            if (profile_dir)
                cfg.ort_profile_dir = profile_dir;
            return new pocket_tts::PocketTTS(cfg);
        }
        catch (const std::exception &e)
        {
    ...
    {
        if (handle)
            static_cast<pocket_tts::PocketTTS *>(handle)->set_soften_commas(soften != 0);
    }
    ...
    {
        if (!stream_ctx || !out_samples || !out_len)
            return -1;
    ...
    int ptt_stream_error(void *stream_ctx, char *msg_buf, int msg_buf_len)
    {
        ...
        if (msg_buf && msg_buf_len > 0)
        {
            ...
            if (n > static_cast<size_t>(msg_buf_len - 1))
                n = static_cast<size_t>(msg_buf_len - 1);
    ...
    {
        if (!latents_ctx || !out_frame || !out_flags)
            return -1;
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (a == "-h" || a == "--help")
        {
            ...
            cfg.lsd_steps = std::stoi(next());
        else if (a == "--threads")
            cfg.num_threads = std::stoi(next());
        else if (a == "--threads-ar")
            cfg.num_threads_ar = std::stoi(next());
        else if (a == "--threads-dec")
            cfg.num_threads_dec = std::stoi(next());
        else if (a == "--threads-full")
            cfg.num_threads_full = std::stoi(next());
        else if (a == "--models-dir")
            cfg.models_dir = next();
        else if (a == "--voices-dir")
            cfg.voices_dir = next();
        else if (a == "--tokenizer")
            cfg.tokenizer_path = next();
        else if (a == "--ort-profile-dir")
            cfg.ort_profile_dir = next();
        else if (a == "--eos-threshold")
            cfg.eos_threshold = std::stof(next());
        else if (a == "--noise-clamp")
            cfg.noise_clamp = std::stof(next());
        else if (a == "--eos-extra")
            cfg.eos_extra_frames = std::stoi(next());
        else if (a == "--first-chunk")
        {
            cfg.first_chunk_frames = std::stoi(next());
            first_chunk_set = true;
        }
        else if (a == "--max-chunk")
            cfg.max_chunk_frames = std::stoi(next());
        else if (a == "--no-cache")
            cfg.voice_cache = false;
        else if (a == "--keep-commas")
            cfg.soften_commas = false;
        else if (a == "--fast-start")
            cfg.fast_start = true;
        else if (a == "--low-latency")
            cfg.low_latency = true;
        else if (a == "--trim-leading")
            cfg.force_leading_trim = true;
        else if (a == "--trim-confirm")
            cfg.leading_confirm_frames = std::max(1, std::stoi(next()));
        else if (a == "--trim-drop")
            cfg.leading_drop_samples = std::max(0, std::stoi(next()));
        else if (a == "--trim-preroll")
            cfg.leading_preroll_frames = std::max(0, std::stoi(next()));
        else if (a == "--trim-sustain")
            cfg.leading_sustain_frames = std::max(0, std::stoi(next()));
        else if (a == "--trim-collapse")
            cfg.leading_collapse_frames = std::max(1, std::stoi(next()));
        else if (a == "--no-delta-kv")
            cfg.delta_kv = false;
        else if (a == "--no-custom-attention")
            cfg.custom_attention = false;
        else if (a == "--decoder-convtr")
            cfg.decoder_convtr = true;
        else if (a == "--no-decoder-convtr")
            cfg.decoder_convtr = false;
        else if (a == "--no-merged-flow")
            cfg.merged_flow = false;
        else if (a == "--combine-first-step")
            cfg.combine_first_step = true;
        else if (a == "--separate-first-step")
            cfg.combine_first_step = false;
        else if (a == "--stdout")
            stdout_output = true;
        else if (a == "--verbose")
            cfg.verbose = true;
        else if (a == "--profile")
            pocket_tts::g_prof.enabled = true;
        else if (a == "--server")
            server_mode = true;
        else if (a == "--port")
            server_port = std::stoi(next());
        else if (a[0] == '-')
        {
            std::cerr << "Unknown: " << a << "\n";
            return 1;
