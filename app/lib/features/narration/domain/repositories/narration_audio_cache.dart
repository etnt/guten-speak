...
abstract interface class NarrationAudioCache {
  ...
  /// `(bookId, voiceId)` pair, or null if it is missing or stale (its text hash
  /// no longer matches). A stale/missing entry is treated as not cached so the
  /// scheduler re-renders it.
  Future<String?> cachedPath(int bookId, String voiceId, NarrationUnit unit);

  /// Ensures the parent directory exists and returns the path a fresh clip for
  /// `(bookId, voiceId, unitIndex)` should be written to.
  Future<String> reservePath(int bookId, String voiceId, int unitIndex);
  ...
  Future<void> record(int bookId, String voiceId, NarrationUnit unit);

  /// Rolling-window eviction: deletes cached clips and index rows for this
  /// `(bookId, voiceId)` pair whose unit index falls outside `[lo, hi]`.
