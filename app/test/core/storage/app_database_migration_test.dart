...
void main() {
  ...
  test(
    'fresh v6 database has a profile-aware synth_cache primary key',
    () async {
      ...
      final columns = await db.rawQuery('PRAGMA table_info(${Db.synthCache})');
      final pkColumns = columns
          .where((c) => (c['pk'] as int) > 0)
          .map((c) => c['name'])
          .toSet();

      expect(pkColumns, <String>{
        Db.synthBookId,
        Db.synthVoiceId,
        Db.synthUnitIndex,
        Db.synthProfileId,
  ...
  test(
    'v5→v6 migration recreates a profile-aware, empty synth_cache',
    () async {
  ...
  test('two profiles can coexist for the same book/voice/unit', () async {
    ...
    for (final profileId in <String>['legacy-profile-id', 'raven-profile-id']) {
    ...
    expect(rows.map((r) => r[Db.synthProfileId]).toSet(), <String>{
      'legacy-profile-id',
      'raven-profile-id',
    });
  });
