...
///
/// [builtIn] voices ship with the app (e.g. the bundled sample) and cannot be
/// deleted; user voices are imported `.wav` files copied into app storage.
class Voice {
  const Voice({
    required this.id,
    required this.name,
...
/// `index.json` mapping ids → names; the built-in sample is injected at load
...
class VoiceLibrary {
  ...
  _builtInAssets = [
    (
      ...
      asset: 'assets/reginald-ashworth.wav',
      file: 'reginald-ashworth.wav',
    ),
    (
      id: '__builtin_deja__',
      name: 'Deja Thoris (female)',
      asset: 'assets/deja-thoris.wav',
